...
const setChirpyRedActive = `-- name: SetChirpyRedActive :one
UPDATE users
...
func (q *Queries) SetChirpyRedActive(ctx context.Context, id uuid.UUID) (User, error) {
	row := q.db.QueryRowContext(ctx, setChirpyRedActive, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
